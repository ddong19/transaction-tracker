...
import { ScheduledTransactionsView } from './scheduled-transactions-view';
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        {activeTab === 'transactions' ? (
          ...
          <div className="pt-4">
            <ScheduledTransactionsView
              ...
              onToggleEnabled={onToggleScheduledEnabled}
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-white via-white to-transparent pointer-events-none">
        <Button
          onClick={activeTab === 'transactions' ? onAddTransaction : onAddScheduledTransaction}
          className="w-full bg-slate-900 hover:bg-slate-800 text-white rounded-xl h-14 text-base shadow-lg pointer-events-auto"
        >
          <Plus className="w-5 h-5 mr-2" />
          {activeTab === 'transactions' ? 'Add Transaction' : 'Schedule Expense'}
        </Button>
      </div>
    </div>
  );
